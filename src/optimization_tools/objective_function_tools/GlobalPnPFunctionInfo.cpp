...
GlobalPnPFunctionInfo::GlobalPnPFunctionInfo(const opengv::absolute_pose::AbsoluteAdapterBase & adapter ){
  ...
  Eigen::MatrixXd Mr2_i = Eigen::MatrixXd::Zero(1,9);
  ...
  for( int i = 0; i < total_points; i++ )
  {
    ...
    Mr2_i.block<1,3>(0,0) = vi(0,0) * xi.transpose();
    Mr2_i.block<1,3>(0,3) = vi(1,0) * xi.transpose();
    Mr2_i.block<1,3>(0,6) = vi(2,0) * xi.transpose();
...
double GlobalPnPFunctionInfo::objective_function_value(const opengv::rotation_t & rotation, const opengv::translation_t & translation){return 0.0;}
