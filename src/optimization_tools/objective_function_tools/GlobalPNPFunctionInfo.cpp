...
GlobalPNPFunctionInfo:: GlobalPNPFunctionInfo(const opengv::absolute_pose::AbsoluteAdapterBase & adapter ){
  ...
  for( int i = 0; i < (int) indices.size(); i++ )
  {
    ...
    Mrt_i.block<1,3>(1,0) = Qi(1,0) * xi.transpose(); Mrt_i.block<1,3>(1,3) = Qi(0,1) * xi.transpose(); Mrt_i.block<1,3>(1,6) = Qi(0,2) * xi.transpose();
    Mrt_i.block<1,3>(2,0) = Qi(2,0) * xi.transpose(); Mrt_i.block<1,3>(2,3) = Qi(0,1) * xi.transpose(); Mrt_i.block<1,3>(2,6) = Qi(0,2) * xi.transpose();
